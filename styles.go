...
func (f *File) NewStyle(style string) (int, error) {
	...
	if fs.Font != nil {
		font, _ := xml.Marshal(f.setFont(fs))
		s.Fonts.Count++
		s.Fonts.Font = append(s.Fonts.Font, &xlsxFont{
			Font: stripFontBytes(font),
		})
...
// stripFontBytes removes the wrapping <font> tags from the marshalled
// xml value
func stripFontBytes(fontBytes []byte) string {
	return string(fontBytes[6 : len(fontBytes)-7])
}
...
func (f *File) SetDefaultFont(fontName string) {
	...
	fontBytes, _ := xml.Marshal(font)
	s := f.stylesReader()
	s.Fonts.Font[0].Font = stripFontBytes(fontBytes)
	custom := true
	s.CellStyles.CellStyle[0].CustomBuiltIn = &custom
}

// readDefaultFont provides an unmarshalled font value
func (f *File) readDefaultFont() font {
	s := f.stylesReader()
	var font font
	err := xml.Unmarshal([]byte("<font>"+s.Fonts.Font[0].Font+"</font>"), &font)
	if err != nil {
		panic("unable to read default font")
	}
	return font
}
...
// settings.
func (f *File) setFont(formatStyle *formatStyle) *font {
